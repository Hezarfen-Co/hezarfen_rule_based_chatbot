...
def collect_samples(
    ...
    """Her örnek için (güven, top-1, doğru mu, in_domain) toplar."""

    matcher = matcher or get_default_matcher()
    records = records if records is not None else load_benchmark()

    in_scope: list[dict[str, Any]] = []   # {conf, correct}
    oos: list[dict[str, Any]] = []        # {conf, in_domain, accepted}
    ...
    for r in records:
        ...
        rule_hit = trace["rule_layer"]["hit"]
        in_domain = trace["similarity"].get("in_domain", True)
        expected = r["expected_intent"]

        if expected == OOS_LABEL:
            # Kabul (accept) = kural çarptı VEYA (alan-içi ve güven >= eşik).
            accepted = rule_hit or (in_domain and conf >= DEFAULT_THRESHOLD)
            oos.append({"conf": conf, "in_domain": in_domain, "accepted": accepted})
        else:
            in_scope.append({"conf": conf, "correct": top1 == expected})
...
def compute(
    ...
    # İşletim noktası: güven >= DEFAULT_THRESHOLD olanları cevapla (kural=1.0 hep dahil).
    answered = [s for s in in_scope if s["conf"] >= DEFAULT_THRESHOLD]
    answered_wrong = sum(1 for s in answered if not s["correct"])
